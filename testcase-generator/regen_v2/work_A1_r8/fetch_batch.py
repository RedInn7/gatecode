...
def load_done_pids():
    ...
    for fname in ('skipped_by_A1.json', 'skipped_by_A2.json'):
        try:
            with open(os.path.join(REGEN_DIR, fname)) as fh:
...
if __name__ == '__main__':
    start = int(sys.argv[1]) if len(sys.argv) > 1 else 549
    end = int(sys.argv[2]) if len(sys.argv) > 2 else 1791
